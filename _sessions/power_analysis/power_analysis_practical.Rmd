...
3. According to the power analysis, a sample of 175 individuals would have been necessary. Visualize this result using the `plot.power.htest()` function. You can simply apply on the `sample_size` object. 
...
5. To determine the power of a completed study based on the empirical effect size is called a post-hoc power analysis. The `pwr.t.test` also allows you to carry such analyses. Simply specify `n` rather than power. Try it out and determine the exact post-hoc power value for Bem's empirical effect size.  
...
7. For very small effects Bem's study only had a power of `25.7`%. So, given that Bem couldn't know that his effect would be as large as `d = .25`, should probably have conducted a larger study? How large should his sample have been to detect effects of size `d = .1` with power `.95`.     
...
3. Now let's assume actually wanted to test whether there is a difference in the hit rate for `erotic` between men and women. Let's assume the the expected effect size is again `d = .1`, what is the sample needed for $alpha = .05$ and $power = .95$. To reflect the difference in design set `type = "two.sample"` in the `pwr.t.test()` function.  
...
4. Given that there now is a true effect, your are observing power, not $\alpha$, and power is rather small. You can confirm this result using the `pwr.t.test()` function. See below.  
